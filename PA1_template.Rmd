...
```{r, echo=TRUE}
mean.intervalsteps <- tapply(pasos$steps, pasos$interval, function(x) mean(x, na.rm=T))
xrange <- 1:length(mean.intervalsteps)
plot( xrange, mean.intervalsteps, type = "l", main="time series", xlab="5 minute interval", ylab="Average Number of steps")
...
```{r, echo=TRUE}
max.mean <- max(mean.intervalsteps)
highest.interval <- mean.intervalsteps[xrange[mean.intervalsteps==max.mean]]
highest.interval
```

The 5-minute interval is 835 with the high average number of steps of 206.16
...
for (i in 1:nrow(pasos)) {
    if (is.na(pasos$steps[i])) {
        copiapasos$steps[i] <- mean.intervalsteps[as.character(pasos$interval[i])]
    }
}
```
